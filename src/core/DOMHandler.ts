 ...
 * before using it.
 *
 * @param html // string form
 * @param fileMode // set to true if the content contains `head` tag
 * @param nodeParser
 *
 * The custom nodeParser is expected to return the body part.
 */
export const defaultDomFactory: DomFactory = async (html, fileMode: boolean = false, nodeParser?: DomFactory) => {
    ...
    try {
        return await nodeParser(html, fileMode)
    } catch (e) {
